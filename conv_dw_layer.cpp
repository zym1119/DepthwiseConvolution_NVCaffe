...
void ConvolutionDepthwiseLayer<Ftype, Btype>::LayerSetUp(const vector<Blob*>& bottom,
  ...
  if (this->blobs_.size() == 0) {
    ...
    shared_ptr<Filler<Ftype> > weight_filler(GetFiller<Ftype>(conv_param.weight_filler()));
    weight_filler->Fill(this->blobs_[0].get());
    if (conv_param.bias_term()) {
      this->blobs_[1].reset(new TBlob<Ftype>(bias_shape));
      shared_ptr<Filler<Ftype> > bias_filler(GetFiller<Ftype>(conv_param.bias_filler()));
...
template <typename Ftype, typename Btype>
void ConvolutionDepthwiseLayer<Ftype, Btype>::Reshape(const vector<Blob*>& bottom,
      const vector<Blob*>& top) {
  ...
  caffe_gpu_set(weight_multiplier_.count(), Ftype(1), weight_multiplier_.mutable_gpu_data());
  if (this->layer_param_.convolution_param().bias_term())
  {
    ...
    caffe_gpu_set(bias_multiplier_.count(), Ftype(1), bias_multiplier_.mutable_gpu_data());
  }
}

template <typename Ftype, typename Btype>
void ConvolutionDepthwiseLayer<Ftype, Btype>::Forward_cpu(const vector<Blob*>& bottom,
      const vector<Blob*>& top)
{
  ...
  const Ftype* weight_data_base = this->blobs_[0]->template cpu_data<Ftype>();
  ...
  for (int n = 0; n < num; ++n)
  {
    for (int c = 0; c < channels; ++c)
    {
      for (int h = 0; h < top_height; ++h)
      {
        ...
        {
          const Ftype* weight_data = weight_data_base + c * kernel_h_ * kernel_w_;
          Ftype value = 0;
  ...
  if (this->layer_param_.convolution_param().bias_term())
  {
    ...
    for (int n = 0; n < num; ++n)
    {
      const Ftype* bias_data = this->blobs_[1]->template cpu_data<Ftype>();
      for (int c = 0; c < channels; ++c)
      {
        for (int h = 0; h < top_height; ++h)
        {
...
template <typename Ftype, typename Btype>
void ConvolutionDepthwiseLayer<Ftype, Btype>::Backward_cpu(const vector<Blob*>& top,
      const vector<bool>& propagate_down, const vector<Blob*>& bottom)
{
  ...
  const int bottom_height = bottom[0]->height();
  const int bottom_width = bottom[0]->width();
  caffe_set(bottom[0]->count(), Btype(0), bottom[0]->mutable_cpu_diff<Btype>());
  ...
  if (this->param_propagate_down_[0])
  {
    ...
    Btype* weight_diff_base = this->blobs_[0]->template mutable_cpu_diff<Btype>();
    for (int n = 0; n < num; ++n)
    {
      for (int c = 0; c < channels; ++c)
      {
        for (int h = 0; h < top_height; ++h)
        {
          for (int w = 0; w < top_width; ++w)
          {
            Btype* weight_diff = weight_diff_base + c * kernel_h_ * kernel_w_;
            for (int kh = 0; kh < kernel_h_; ++kh)
            {
              for (int kw = 0; kw < kernel_w_; ++kw)
              {
  ...
  {
    const Btype* top_diff = top[0]->cpu_diff<Btype>();
    const Btype* weight_data_base = this->blobs_[0]->template cpu_data<Btype>();
    ...
    for (int n = 0; n < num; ++n)
    {
      for (int c = 0; c < channels; ++c)
      {
        for (int h = 0; h < top_height; ++h)
        {
          for (int w = 0; w < top_width; ++w)
          {
            const Btype* weight_data = weight_data_base + c * kernel_h_ * kernel_w_;
            for (int kh = 0; kh < kernel_h_; ++kh)
            {
              for (int kw = 0; kw < kernel_w_; ++kw)
              {
...
INSTANTIATE_CLASS_FB(ConvolutionDepthwiseLayer);
